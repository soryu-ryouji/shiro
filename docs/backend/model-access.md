...
- **配置界面**：Model 面板按功能分层——注册分区「模型导入」（供应商 + 模型可检索下拉 + API Key 三行表单，保存即注册/覆盖同 key 档案）；管理分区「模型管理」（已注册档案列表：供应商 · 当前模型 · Key 掩码状态，可设默认供应商 / 编辑 / 删除）。供应商预设含完整内置模型清单（学 pi 内置目录随版本固化，来源 pi providers/data），端点与协议是预设内部实现，不暴露给用户
