...
**生成与通用化。** 资产成为通用物的前提是结构上分层：

- **可迁移层（模板）**：性格原型、动机结构、说话风格、功能角色（对手 / 盟友 / 导师）、成长弧线类型——不绑定任何一本书
- **实例层**：名字、外貌、本作身世、与具体角色的关系、当前状态——绑定本项目

frontmatter 存结构化字段与来源标注（如 `source: db://characters/xxx`），正文按两区块组织；字段表、区块标题、文件命名等格式细则见 [资产格式](./asset-format.md)。

资产的来源只有四条，全部收敛到同一规则——AI 产物走草稿→确认，确认后成为明文资产文件：

| 来源 | 路径 |
| ---- | ---- |
| 管道生成 | 角色准备等节点的产物即资产草稿，确认后写入 `设定/` |
| 拆书 | 外部文本导入 → AI 拆解出人物 / 设定 / 套路 → 草稿 → 确认入库；拆书产物默认泛化后进全局库（学到的是原型与套路，顺便去掉原作专名） |
| 手写 | 资产即 markdown 文件，直接编辑 |
| 应用 | 从全局库应用到项目（见引用语义） |

「应用到项目」「发布到全局库」支持可选的 AI 辅助转换，纯复制手动改写仍然成立：

- **应用 + 实例化**：原型 + 本项目世界观 → 生成本作实例字段（名字、身世适配新世界）
- **发布 + 泛化**：去掉专名与本作事件，把实例描写提炼为可迁移结构（性格、说话风格、弧线），存为全局模板

不参与通用化的资产：时间线、信息披露状态、伏笔台账是本作事实，只存在于项目内；情绪曲线本身不通用，但节拍模板（三幕、救猫咪等）可作为全局库条目。
